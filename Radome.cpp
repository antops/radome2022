	...
	: QMainWindow(parent)
{
	...
	aCamera->SetViewUp(0, 0, 1);//?????¶À?? 
	aCamera->SetPosition(0, -3 * axesScale, 0);//???????¶À
	aCamera->SetFocalPoint(0, 0, 0);//?Èˆ?? 
...
void Radome::UpdateRotateRadome() {
	...
	if (old_radome) {
		QMessageBox::StandardButton rb =
			QMessageBox::question(NULL, "question", QString::fromLocal8Bit("??????????????????ùI"),
...
void Radome::OnNewQuickCalcalteAction()
{
	...
	process_show_widget_ = new ProcessShowWidget;
	process_show_widget_->SetWidgetText(QString::fromLocal8Bit("??????ß·?????????????????..."));
	process_show_widget_->setWindowFlags(Qt::WindowStaysOnTopHint);
	process_show_widget_->resize(800, 600);
	// process_show_widget_->SetTextBrowserHidden(true);
	process_show_widget_->show();
...
void Radome::OnNewGAOptAction() {
	...
	if (results_ifs.is_open()) {
		string temp;
		int lineNumber = 0;
		while (getline(results_ifs, temp)) { // ???ßÿ?????  
			lineNumber++; // ?????ß‹??ß‹??1  
...
void Radome::onLoadProject() {
	...
	results_filesave = stl_load.GetResults();//???stl???°§?????????results_filesave??????¶¡???
	...
	for (int i = 0; i < index_material.size(); i++) {
			...
			data_manager_.SetMaterialData(index_material[i], data_material_);//????????????material_book?ß≥?
