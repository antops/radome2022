...
void TaileSourceWidget::InitSourceParam() {
	...
	basic_qbox_ = new QGroupBox(QString::fromLocal8Bit("��������"));
	basic_qbox_->setLayout(basic_layout);
}

TaileSourceWidget::~TaileSourceWidget()
{
	
}

bool TaileSourceWidget::getField(Field *& ptr)
{
	if (!GetGraphTransOk()) {
		return false;
	}
	if (!GenSource()) {
		QMessageBox::warning(NULL, "Warning", QString::fromLocal8Bit("����Դʧ��"));
		return false;
	}
	bool ok, ok_back;
	...
	treeWidth = new QTreeWidgetItem;
	treeWidth->setText(0, QString::fromLocal8Bit("x��������Ԫ���(����):") + QString::number(dx_));
	tree->addChild(treeWidth);

	treeWidth = new QTreeWidgetItem;
	treeWidth->setText(0, QString::fromLocal8Bit("y��������Ԫ���(����):") + QString::number(dy_));
	tree->addChild(treeWidth);
...
void TaileSourceWidget::setMirror(Mirror * ptr)
{
...
void TaileSourceWidget::on_widthChange(QString var)
{
	bool ok = false;
...
void TaileSourceWidget::on_depthChange(QString var)
{
	bool ok = false;
	double res = var.toDouble(&ok);
	if (!ok)
...
bool TaileSourceWidget::ReadScanParam() {
...
void TaileSourceWidget::GenSourceMeta() {
...
bool TaileSourceWidget::IsPythonWorkSuccess(const std::string& status_file) {
	std::ifstream ifs;
...
bool TaileSourceWidget::GenSource() {
	if (!ReadScanParam()) {
		QMessageBox::warning(NULL, "Warning", QString::fromLocal8Bit("��д��������"));
		return false;
	}
...
void TaileSourceWidget::SaveParam(Json::Value* param_js_ptr) {
	...
	SAVE_EDIT_TO_JSON(theta_edit_, param_js);
	SAVE_EDIT_TO_JSON(phi_edit_, param_js);
	SAVE_EDIT_TO_JSON(fre_edit_, param_js);
	SAVE_EDIT_TO_JSON(radius_edit_, param_js);
	SAVE_EDIT_TO_JSON(s11_edit_, param_js);
	SAVE_EDIT_TO_JSON(dx_edit_, param_js);
	SAVE_EDIT_TO_JSON(dy_edit_, param_js);
	SAVE_EDIT_TO_JSON(ds_edit_, param_js);

	GraphTransWidget::SaveParam(param_js_ptr);
}

bool TaileSourceWidget::LoadParam(const Json::Value& param_js) {
	try
	{
		READ_EDIT_TO_JSON(theta_edit_, param_js);
		READ_EDIT_TO_JSON(phi_edit_, param_js);
		READ_EDIT_TO_JSON(fre_edit_, param_js);
		READ_EDIT_TO_JSON(radius_edit_, param_js);
		READ_EDIT_TO_JSON(s11_edit_, param_js);
		READ_EDIT_TO_JSON(dx_edit_, param_js);
		READ_EDIT_TO_JSON(dy_edit_, param_js);
		READ_EDIT_TO_JSON(ds_edit_, param_js);

	}
	catch (const std::exception&)
	{
		QMessageBox::warning(NULL, "Warning",
			QString::fromLocal8Bit("��ȡ GraphTransWidget ʧ��"));
		return false;
	}
