...
from initial_input import initSource_input
from non_radome import calculate_nonFarField_new
from non_radome import calculate_nonFarField
...
AxisN=np.array([0., 0, 1.])
AxisU=np.array([1., 0, 0.])
AxisV=np.array([0., 1, 0.])
OutOrigin=np.array([0., 0, 0.])

originX=content['source_pos']['originX']
originY=content['source_pos']['originY']
originZ=content['source_pos']['originZ']

rotU=content['source_pos']['rotU']
rotV=content['source_pos']['rotV']
rotN=content['source_pos']['rotN']

rotUnum=content['source_pos']['rotUnum']
rotVnum=content['source_pos']['rotVnum']
rotNnum=content['source_pos']['rotNnum']

xx=rotU* np.pi/180.0
yy=rotV* np.pi/180.0
zz=rotN* np.pi/180.0
matx = np.array([[1, 0, 0], [0, np.cos(xx), -np.sin(xx)], [0, np.sin(xx), np.cos(xx)]])
maty = np.array([[np.cos(yy), 0, np.sin(yy)], [0, 1, 0], [-np.sin(yy), 0, np.cos(yy)]])
matz = np.array([[np.cos(zz), -np.sin(zz), 0], [np.sin(zz), np.cos(zz), 0], [0, 0, 1]])
matRot=np.zeros((3,3,3),dtype=float)
matRot[:,:,rotUnum]=matx
matRot[:,:,rotVnum]=maty
matRot[:,:,rotNnum]=matz
matxy=np.dot(matx,maty)
mat12=np.dot(matRot[:,:,0],matRot[:,:,1])
mat123=np.dot(mat12,matRot[:,:,2])
# mat_rot=np.dot(matxyz,mat.T)
InOrigin=[originX,originY,originZ]
UU=np.array([1,0,0])
InAxisU=np.dot(mat123,UU.T)
VV=np.array([0,1,0])
InAxisV=np.dot(mat123,VV.T)
InAxisN=np.cross(InAxisU,InAxisV)
#加载入射场
...
elif polarization_mode == 0:
    FieldInput = initSource(sourcepath,polarization_mode)
    FieldInput=FieldInput[:,:,0]  #水平极化，Ex有值
else:
    FieldInput,read_mode = initSource_input(sourcepath)
    polarization_mode=read_mode
...
minphi=content['farfield']['minphi']
mintheta=content['farfield']['mintheta']
angle_phi=0 #波束指向方位角
theta = np.linspace(mintheta, maxtheta, num=numtheta)
theta = theta * np.pi/180.0
phi = np.linspace(minphi, maxphi, num=numphi)
phi = phi * np.pi / 180.0
#入射场网格化，画图
...
#内表面坐标系变换到馈源
cellcenterLocal=AGB.ToLocalCoordinate(cellcenter, InAxisU, InAxisV, InAxisN, InOrigin)
# index_above=np.argwhere(cellcenterLocal[:,2]>0).ravel()
# cellcenterLocal=cellcenterLocal[index_above,:]
# 计算内表面被照射场分布
if polarization_mode == 1:
    CalcFlag = np.array([0, 1, 1, 1, 1, 1])
else:
    CalcFlag=np.array([1, 0, 1, 1, 1, 1])
FieldOnRadome=AGB.FieldOnSurface(k0,np.loadtxt(syspath + "\\2AdaptiveGB\\GaussInfo.txt"),cellcenterLocal,syspath + "\\2AdaptiveGB\\FieldOnRadome.npy",syspath + "\\2AdaptiveGB\\PoyntingVectors.npy",CalcFlag) #输出全部电磁场分布文件
FieldOnRadomeX=FieldOnRadome[:,0]
FieldOnRadomeY=FieldOnRadome[:,1]
fdx,fdy = np.where(np.isnan(FieldOnRadome))
# print("Done",flush=True)

InAxis=np.array([InAxisU, InAxisV, InAxisN]) #入射场坐标系
Axis=np.array([AxisU,AxisV,AxisN]) #世界坐标系
In_0TransAxis=np.dot(InAxis,np.linalg.inv(Axis)) #两个坐标系的变换关系，从源坐标到世界坐标

PoyntingVectorLocal=np.load(syspath + "\\2AdaptiveGB\\PoyntingVectors.npy").astype(np.float32)
PoyntingVectors=AGB.ToWorldCoordinate(PoyntingVectorLocal, InAxisU, InAxisV, InAxisN, InOrigin)
...
# 分电场磁场转换矢量，合并保存
FieldExEyEz=np.dot(np.load(syspath + "\\2AdaptiveGB\\FieldOnRadome.npy") [:,:3],In_0TransAxis) #曲面坐标系的矢量变换到出射面坐标系的矢量，矢量本身不变，正交分量变化
# FieldRefH=np.dot(np.load(syspath + "\\2AdaptiveGB\\FieldOnRadome.npy") [:,3:],In_0TransAxis) #曲面坐标系的矢量变换到出射面坐标系的矢量，矢量本身不变，正交分量变化
#初始化入射光线
Etile = np.tile(np.array(2).transpose(), (1, 3))
numofRay=len(IndexPV)
# incfield=FieldExEyEz[IndexPV]
incfield=FieldExEyEz[IndexPV]/Etile
# cellcenter=cellcenter[index_above,:]
# cellarea_inner=cellarea_inner[index_above]
...
# norm_tri_inner=norm_tri_inner[index_above]
...
dtheta = (maxtheta-mintheta)/(numtheta-1)
dphi = (maxphi-minphi)/(numphi-1)
...
avePY =  np.sum((pow(np.abs(Es[:,:,0]),2)+pow(np.abs(Es[:,:,1]),2)+pow(np.abs(Es[:,:,2]),2))*np.abs(np.sin(theaxx))*dtheta*dphi*np.pi*np.pi/(np.pi*4*180*180))
...
bb1 = np.flip(dbiPY[int(angle_phi/dphi+180/dphi),1:numtheta])
bb2 = dbiPY[int(angle_phi//dphi),0:numtheta]
...
if content["output"]["is_calc_nonradome"]==1:
    ...
    # fieldinputori=initSource(sourcepath,polarization_mode)
    # calculate_nonFarField(fieldinputori, ds, f0, maxtheta, numtheta, maxphi, numphi, outputpath_non)
...
if f0<35e9:
    ...
    tbl = pow((ra_max / non_max),2) * 100
tblsave=np.ones(3)*tbl
np.savetxt(outputpath + '/tbl='+str(tbl)+'.txt', tblsave)
