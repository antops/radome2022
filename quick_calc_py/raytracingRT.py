...
def rayRT(incfield,incorig,incvec,incINDEX,incarea,allLayerInfo,allmedia,k0,depth,maxDepth,numofLayer,numofRay, threads_per_block):
    ...
    if depth != maxDepth and len(ray_INDEX) != 0:
        ...
        if len(inner_origINDEX)>numofRay/10 and len(inner_hitINDEX)>numofRay/10:
            stopTraceflg=1
    ...
    if depth==0:
        ...
        if numref > numofRay*0.02 and stopTraceflg==0:
        ...
        if numtra > numofRay*0.02 and stopTraceflg==0:
