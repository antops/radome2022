...
def polydataReflectorActor(FieldSource,polydataReflector): # (取layers=0, X, Y, 6) 
    ...
    #vtk_FieldVector = numpy_to_vtk(np.real(FieldSource), deep=True, array_type=vtk.VTK_DOUBLE)
    #polydataReflector.GetOutput().GetPointData().SetVectors(vtk_FieldVector) #场值数据赋值给points的Vectors去显示
...
def PatternWorldActor(Farfield0,Farfield1,Farfield2,Thetamesh,Phimesh,FitAxisU,FitAxisV,FitAxisN,FitOrigin,OffsetOrigin):
